...
describe RakeDependencies::Tasks::Fetch do
  ...
  def define_task(opts = {}, &block)
    ...
    additional_tasks = opts[:additional_tasks] || [:download, :extract]
  ...
  context 'parameters' do
    ...
    it 'allows the download task to be overridden' do
      define_task(additional_tasks: [:dl, :extract]) do |t|
    ...
    it 'allows the extract task to be overridden' do
      define_task(additional_tasks: [:download, :ex]) do |t|
      ...
      expect(Rake::Task['dependency:download']).to(receive(:invoke).ordered)
      expect(Rake::Task['dependency:ex']).to(receive(:invoke).ordered)
    ...
    it 'uses the correct namespace for prerequisites when multiple fetch tasks are declared' do
      ...
      expect(Rake::Task['dependency1:download']).to(receive(:invoke).ordered)
      expect(Rake::Task['dependency1:extract']).to(receive(:invoke).ordered)

      dependency1_task.invoke

      expect(Rake::Task['dependency2:download']).to(receive(:invoke).ordered)
      expect(Rake::Task['dependency2:extract']).to(receive(:invoke).ordered)
